...
import { ModalService } from '@kms-frontend/ui/modal';
...
export const BoardStore = signalStore(
  ...
  withMethods(
    (
      ...
      createIssue: rxMethod<{ payload: CreateIssueRequest }>(
        pipe(
          tap(() => patchState(store, { isLoading: true })),
          switchMap(({ payload }) => {
            return boardService.createIssue(payload).pipe(
              switchMap((createIssueResponse: DefaultResponse) => {
                if (!createIssueResponse.error) {
                  return boardService.getIssues(payload.project).pipe(
                    tapResponse({
                      next: (updatedIssues: Issue[]) => {
                        patchState(store, {
                          data: {
                            ...store.data()!,
